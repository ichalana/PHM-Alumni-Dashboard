...
if dropped:
    st.caption(f"Dropped {dropped} row(s) with unknown location, job, or company.")
...
c1.metric("Alumni", f"{len(filtered):,}")
c2.metric("Unique Companies", f"{filtered['company'].nunique():,}")
c3.metric("Unique Locations", f"{filtered['location'].nunique():,}")
c4.metric("Job Categories", f"{filtered['job_category'].nunique():,}")
